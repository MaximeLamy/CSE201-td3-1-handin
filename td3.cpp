...
double* shrink_array(double* array, int length, int new_size) {
  // IMPLEMENT YOUR FUNCTION HERE
    double* shrunk_array = new double[new_size];

    for (int i = 0; i < new_size; i++) {
        shrunk_array[i] = array[i];
    }

    delete array;

  return shrunk_array; // YOU CAN CHANGE THIS
}
...
double* append_to_array(double element,
  ...
  // IMPLEMENT YOUR FUNCTION HERE
    if (current_size == max_size) {
        double* appended = extend_array(array, max_size, max_size + 5);
        appended[current_size] = element;
        current_size++;
        max_size += 5;
        return appended;
    }
...
double* remove_from_array(double* array,
  ...
  // IMPLEMENT YOUR FUNCTION HERE
    ...
    if (current_size > 0) array[current_size] = 0;
    else current_size = 0;

    if ((max_size - current_size >= 5) || (current_size - max_size >= 5)) {
    ...
    for (int i = 0; i < telemetry_current_size - 3; i += 3) {
        for (int j = 0; j < telemetry_current_size - (i+3); j+= 3) {
            if (telemetry[j] > telemetry[j+3]) {
                double temp1 = telemetry[j];
                double temp2 = telemetry[j+1];
                double temp3 = telemetry[j+2];
